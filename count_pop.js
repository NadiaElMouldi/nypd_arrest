class bars_pop {
    ...
    const data =  state.pop_race.filter(d => d.GRADE === grade)
    ...
    var colorScale = d3.scaleOrdinal().range([ '#237b80','#cb1010','#eae0d6','#45239a']);
    this.svg = d3.select("#bars_pop")
      .append("svg")
    ...
    const rect = this.svg
      ...
      .join("rect")
      .attr("y", d => yScale(d.n))
      .attr("x", d => xScale(d.POP_RACE) + this.margins.left)
  ...
  // .attr("x", d => xScale(d.POP_RACE) + (0.8*xScale.bandwidth()))
  // .attr("y", d => yScale(d.n))
  // .text(d => d.n)
  // .attr("dy", "1.25em")
